...
| EFProcedural | 1.0.0 | `EFProceduralRuntime`, `EFProceduralACFURuntime`, `EFProceduralPCGRuntime` Runtime/Default; `EFProceduralEditor` Editor/Default | PCG | 26 | 13 headers; 4 UCLASS, 3 UINTERFACE | exact 20-package contract batch plus exact `DungeonGeneration` World present; dungeon actor/door deferred | MIGRATE_SOURCE | PHASE4_EDITOR_GAME_EXACT_CONTRACTS_AND_MAP_STATIC_PASS / PIE_VISUAL_COOK_PACKAGE_DUNGEON_RUNTIME_PENDING |
| EFLevelFlow | 1.0.0 | `EFLevelFlowRuntime` Runtime/Default | AscentCombatFramework, EFProcedural, EFCharacterCreation | 7 | 3 headers; 2 UCLASS | native defaults; `DungeonGeneration` load/save/reload static gate PASS | REBUILD_AGAINST_TARGET | PHASE4_EDITOR_GAME_AND_MAP_STATIC_PASS / LEVEL_FLOW_PIE_VISUAL_COOK_PACKAGE_PENDING |
